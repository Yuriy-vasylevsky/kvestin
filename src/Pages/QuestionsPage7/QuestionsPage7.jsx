...
import { questions7 } from '../../data/questions7';
import { useState } from 'react';
import { useSelector } from 'react-redux';

export default function QuestionsPage_1() {
  const [usedNumbers, setUsedNumbers] = useState(
    localStorage.getItem(`questions7`)
      ? localStorage
          .getItem(`questions7`)
          .split(',')
          .map(function (item) {
  ...
  const rundomaizer = max => {
    ...
    while (usedNumbers.length <= max) {
      ...
      if (usedNumbers.indexOf(rundomNumber) === -1) {
        ...
        localStorage.setItem(`questions7`, usedNumbers);
        return rundomNumber;
  ...
  const changeQuestion = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      setQuestions(questions7[rundomaizer(questions7.length)]);
      setLoading(false);
    }, 500);
    setQuestions('');
  };
